...
const renderAuthError = (
  ...
  return res
    ...
    .render(view, {...data, error: message});
};
  ...
  loginSubmit: async (req: Request, res: Response) => {
    ...
    if (!parsed.success)
      return renderAuthError(
        res,
        'auth/login',
        new AppError('Informe um e-mail e uma senha válidos.', 400),
        {email: req.body.email ?? '', registered: false},
      );
    try {
      ...
      return renderAuthError(res, 'auth/login', error, {
  ...
  registerSubmit: async (req: Request, res: Response) => {
    ...
    if (!parsed.success)
      return renderAuthError(
        res,
        'auth/register',
        new AppError('Preencha os dados obrigatórios corretamente.', 400),
        {values: req.body},
      );
    try {
