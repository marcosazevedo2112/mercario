...
const SalesService = {
  create: async (data: CreateSaleDTO, tenantId: number, createdBy: number) => {
    ...
    for (const item of data.items) {
      ...
      sale.addItem({tenantId, productId: product.id, productName: product.name, quantity: item.quantity, unitPriceCents: product.priceCents, subtotalCents: product.priceCents * item.quantity});
    }
    sale.setDiscount(data.discountCents);
    sale.setPaymentPlan(PaymentPlan.create({initialDueDate: new Date(data.paymentPlan.initialDueDate), installments: data.paymentPlan.installments, modality: data.paymentPlan.modality, paymentMethod: data.paymentPlan.paymentMethod}));
    const confirmed = sale.confirm();
    const createData: CreateSaleData = {sale: confirmed.sale, items: confirmed.items, installments: confirmed.installments.map(installment => ({tenantId, number: installment.number, amountCents: installment.amountCents, paidAmountCents: installment.paidAmountCents, dueDate: installment.dueDate, status: InstallmentStatus.PENDING, paidAt: null, settlementId: null, notes: null}))};
  ...
  findMany: async (tenantId: number, options: {page: number; limit: number; status?: SaleStatus; customerId?: number; search?: string}) => {
    ...
    return {data: result.rows, pagination: {page: options.page, limit: options.limit, total: result.count, totalPages: Math.ceil(result.count / options.limit)}};
  },
  getReceivables: async ({tenantId, from, to}: {tenantId: number; from: string; to: string}) => {
    const today = isoToday();
    const rows = await SaleRepository.findReceivables({tenantId, from, to});
    const grouped: Record<string, Array<Receivable & {overdueStatus: string}>> = {};
    const summary = {overdueTotal: 0, todayTotal: 0, upcomingTotal: 0};
    for (const installment of rows) {
      const dueDate = String(installment.dueDate).slice(0, 10);
      const overdueStatus = receivableStatus(dueDate, today);
      (grouped[dueDate] ??= []).push(Object.assign(installment, {overdueStatus}));
      if (overdueStatus === 'overdue') summary.overdueTotal += installment.amountCents;
      else if (overdueStatus === 'today') summary.todayTotal += installment.amountCents;
      else summary.upcomingTotal += installment.amountCents;
    }
    return {data: rows, grouped, summary, from, to, today};
  },
  cancel: async (saleId: number, tenantId: number, canceledBy: number, cancelReason: string) => {
    const reason = cancelReason.trim();
    if (!reason) throw new AppError('O motivo do cancelamento é obrigatório', 400);
    const sale = await SaleRepository.findById(saleId, tenantId);
    if (!sale) throw new AppError('Venda não encontrada', 404);
    if (sale.status !== SaleStatus.OPEN) throw new AppError('A venda não pode ser cancelada', 400);
    await sequelize.transaction(transaction => SaleRepository.cancel(saleId, tenantId, canceledBy, reason, transaction));
  },
  registerPayment: async (installmentId: number, saleId: number, tenantId: number, paidAt: Date) => {
    const sale = await SaleRepository.findById(saleId, tenantId);
    if (!sale) throw new AppError('Venda não encontrada', 404);
    if (sale.status !== SaleStatus.OPEN) throw new AppError('Não é possível registrar pagamento em uma venda encerrada', 400);
    ...
    if (installment.status !== InstallmentStatus.PENDING) throw new AppError('A parcela não está pendente', 400);
    return sequelize.transaction(transaction => SaleRepository.registerPayment(installmentId, saleId, tenantId, paidAt, transaction));
  },
