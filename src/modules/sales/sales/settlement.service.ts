...
const SettlementService = {
  settle: async (saleId: number, tenantId: number, data: SettleData) => {
    ...
    if (sale.status !== SaleStatus.OPEN) throw new AppError('A venda não pode ser quitada', 400);

    const remaining = sale.installments
      .filter(item => item.status === InstallmentStatus.PENDING)
      .reduce((sum, item) => sum + item.amountCents, 0);

    if (remaining <= 0) throw new AppError('A venda não possui saldo pendente', 400);
    ...
    return sequelize.transaction(async transaction => {
      const existing = await SettlementRepository.findBySaleId(saleId, tenantId, transaction);
