...
const ChargeService = {
  send: async (
    ...
    if (installment.status !== InstallmentStatus.PENDING)
      throw new AppError('Só é possível cobrar uma parcela pendente', 400);
    if (!data.message.trim())
      throw new AppError('A mensagem da cobrança é obrigatória', 400);
    return ChargeRepository.create({...data, installmentId, tenantId});
  },
  findMany: (saleId: number, installmentId: number, tenantId: number) =>
    Installment.findOne({where: {id: installmentId, saleId, tenantId}}).then(
      installment => {
        if (!installment) throw new AppError('Parcela não encontrada', 404);
